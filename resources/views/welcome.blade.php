...
@section('content')
                    ...
                    <ul class="dropdown-menu p-2" style="max-height: 250px; overflow-y: auto;"
                        ...
                        <li>
                            <label class="dropdown-item d-flex align-items-center gap-2">
                                <input class="form-check-input bahasa-checkbox" type="checkbox" value="Semua Bahasa">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // ----------------------------
            // FUNGSI: BUAT MARKER BAHASA
            // ----------------------------
            function buatMarkerBahasa() {
            ...
            // ----------------------------
            // FUNGSI: LOAD GEOJSON WILAYAH
            // ----------------------------
            function loadWilayahGeoJSON() {
                    ...
                    if (wilayah.file_geojson) {
                        fetch('/' + wilayah.file_geojson)
                            ...
                            .then(data => {
                                ...
                                wilayahLayers[wilayah.nama_wilayah] = geojsonLayer;
                            })
            ...
            // ----------------------------
            // FILTER WILAYAH (GeoJSON)
            // ----------------------------
            document.querySelectorAll(".wilayah-checkbox").forEach(cb => {
                cb.addEventListener("change", function() {
                    const selected = Array.from(document.querySelectorAll(
                        ".wilayah-checkbox:checked")).map(el => el.value);

                    // kalau pilih "Semua Wilayah"
                    if (selected.includes("Semua Wilayah")) {
                        Object.values(wilayahLayers).forEach(layer => map.addLayer(layer));
                    } else {
                        Object.entries(wilayahLayers).forEach(([nama, layer]) => {
                            if (selected.includes(nama)) {
                                map.addLayer(layer);
                            } else {
            ...
            // ----------------------------
            // FILTER BAHASA (MARKER)
            // ----------------------------
            document.querySelectorAll(".bahasa-checkbox").forEach(cb => {
                cb.addEventListener("change", function() {
                    const selected = Array.from(document.querySelectorAll(
                        ".bahasa-checkbox:checked")).map(el => el.value);

                    // kalau pilih "Semua Bahasa"
                    if (selected.includes("Semua Bahasa")) {
                        bahasaMarkers.forEach(obj => map.addLayer(obj.marker));
                    } else {
                        bahasaMarkers.forEach(obj => {
                            if (selected.includes(obj.data.nama_bahasa)) {
