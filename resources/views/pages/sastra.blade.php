...
@section('content')
    ...
    <style>
        ...
        .toggle-btn {
            background: #e9ecef; border: 1px solid #d1d5db;
            ...
            wilayahData.forEach(w => {
                if (w.file_geojson) {
                    fetch('/' + w.file_geojson).then(res => res.json()).then(data => {
                        L.geoJSON(data, {
                            style: { color: "#FF6B6B", weight: 2, opacity: 1, fillOpacity: 0 }
                        }).addTo(map);
                    });
                }
            });
            ...
            sastraList.forEach(s => {
                if (s.lat && s.lng) {
                    ...
                    const customIcon = L.divIcon({
                        ...
                        html: `<div style="background-color:${warna};width:20px;height:20px;border-radius:50%;border:2px solid white;box-shadow:0 0 6px rgba(0,0,0,0.3);"></div>`,
                        iconSize: [20, 20],
                        iconAnchor: [10, 10],
                        popupAnchor: [0, -10]
                    });
                        ...
                        .bindPopup(`
                            <div style="background:linear-gradient(135deg,#ffffff 0%,#f9fafb 100%);
                                ...
                                <div style="border-bottom:1px solid #eee; padding-bottom:6px; margin-bottom:6px;">
                                    <strong style="font-size:16px; color:#1e40af;">${s.nama_sastra}</strong>
                                </div>
