...
<div class="flex items-center gap-3 text-sm font-semibold mb-5">
        ...
        <p class="text-sm font-bold text-default-900">Detail Data Bahasa</p>
    </div>
        ...
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @switch($bahasa->status)
                        @case('Aman')
                            <span class="px-3 py-1 rounded text-white text-sm font-medium"
                                style="background-color: #22C55E;">Aman</span>
                        @break

                        @case('Rentan')
                            <span class="px-3 py-1 rounded text-black text-sm font-medium"
                                style="background-color: #EAB308;">Rentan</span>
                        @break

                        @case('Pasti Terancam Punah')
                            <span class="px-3 py-1 rounded text-white text-sm font-medium" style="background-color: #F59E0B;">Pasti
                                Terancam Punah</span>
                        @break

                        @case('Sangat Terancam Punah')
                            <span class="px-3 py-1 rounded text-white text-sm font-medium" style="background-color: #EF4444;">Sangat
                                Terancam Punah</span>
                        @break

                        @case('Kritis')
                            <span class="px-3 py-1 rounded text-white text-sm font-medium"
                                style="background-color: #7F1D1D;">Kritis</span>
                        @break

                        @default
                            <span class="px-3 py-1 rounded text-black text-sm font-medium" style="background-color: #94A3B8;">Tidak
                                Diketahui</span>
                    @endswitch
                </div>
                ...
                <div>
                    ...
                    <p class="text-base font-semibold text-default-800">
                        @if (!empty($bahasa->dokumentasi_yt))
                            @php
                                $youtubeId = null;

                                if (str_contains($bahasa->dokumentasi_yt, 'youtu.be')) {
                                    preg_match('/youtu\.be\/([^\?]+)/', $bahasa->dokumentasi_yt, $matches);
                                    $youtubeId = $matches[1] ?? null;
                                } elseif (str_contains($bahasa->dokumentasi_yt, 'youtube.com')) {
                                    preg_match('/v=([^\&]+)/', $bahasa->dokumentasi_yt, $matches);
                                    $youtubeId = $matches[1] ?? null;
                                }
                            @endphp
            ...
            <div class="pt-6 border-t border-default-200 flex flex-wrap justify-between items-center gap-3 print:hidden">
                <!-- Tombol Kembali -->
                <div>
                    <a href="{{ route('bahasa.index') }}" class="btn bg-blue-600 text-white flex items-center gap-2">
