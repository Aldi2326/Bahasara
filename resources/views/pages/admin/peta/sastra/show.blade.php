...
<div class="card overflow-hidden">
    <div class="card-header flex justify-between items-center">
        ...
        <a href="{{ route('sastra.index') }}" class="btn bg-gray-600 text-white">Kembali</a>
    </div>
    ...
    <div class="p-6 space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="text-base font-semibold text-default-800">{{ number_format($sastra['jumlah_penutur']) }} Penutur</p>
            </div>
        ...
        <div class="pt-4 border-t border-default-200 flex justify-end space-x-3">
            <a href="{{ route('sastra.edit', $sastra->id) }}" class="btn bg-blue-600 text-white">Edit</a>
            <form action="{{ route('sastra.destroy', $sastra->id) }}" method="POST" class="inline">
                @csrf
                                                @method('DELETE')
                <button type="submit" onclick="return confirm('Yakin hapus data ini?')" class="btn bg-red-600 text-white">Delete</button>
            </form>
        </div>
    </div>
</div>
