...
@section('title', 'namabahasa')
@section('content')

<div class="card overflow-hidden">
    <div class="card-header flex justify-between items-center">
        <h4 class="card-title">Daftar Nama Bahasa</h4>
        <a href="{{ route('namabahasa.create') }}" class="btn bg-danger text-white">Tambah Nama Bahasa</a>
    </div>

    <!-- Search Bar -->
    <div class="px-6 py-4 flex justify-between items-center">
        <form action="{{ route('namabahasa.index') }}" method="GET" class="flex items-center space-x-2 w-full md:w-1/3">
            <input type="text" name="search" value="{{ request('search') }}"
                class="form-input w-full border rounded-md px-3 py-2 text-sm"
                placeholder="Cari nama namabahasa...">
                ...
                <table class="min-w-full divide-y divide-default-200">
                    <thead>
                        <tr>
                            ...
                            <!-- Kolom Nama namabahasa dengan tombol urut -->
                            <th class="px-6 py-3 text-start text-sm text-default-500">
                                <div class="flex items-center gap-1">
                                    ...
                                    @endphp
                                    <a href="{{ route('namabahasa.index', ['search' => request('search'), 'sort' => $sortOrder]) }}"
                        ...
                        <tr class="odd:bg-white even:bg-default-100">
                            ...
                            <td class="px-6 py-4 text-sm text-default-800">
                                <div id="pin-preview" class="flex items-center gap-2">
                                    <svg id="pin-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="30" height="30" fill="{{ $item->warna_pin }}">
                            ...
                            <td class="px-6 py-4 flex justify-end text-sm text-end font-medium space-x-3">
                                <a href="{{ route('namabahasa.edit', $item->id) }}"
                                    class="text-blue-600 hover:underline">Edit</a>
                                <form action="{{ route('namabahasa.destroy', $item->id) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
