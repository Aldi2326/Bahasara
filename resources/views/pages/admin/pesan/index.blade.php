            ...
            <tbody class="divide-y divide-gray-100 text-center">
                @forelse ($kontaks as $index => $kontak)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-gray-800">{{ $kontak->nama }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $kontak->email }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $kontak->subjek }}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex justify-center">
                                <form action="{{ route('kontak.destroy', $kontak->id) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
                                            class="text-red-600 hover:text-red-800 transition btn-delete" 
                                            title="Hapus Pesan">
